...
namespace SageLiveAccess
{
	internal class PullInvoicesService
	{
		...
		{
			var invoiceType = await this._documentTypeHelper.GetSaleInvoiceTypeId();
			...
			{
				// get invoice items
				var items = await this.GetSaleInvoiceItems( rawInvoice );

				// get contact and address info
				var contactAndAddressInfo = await this.GetContactAndAddressInfo( rawInvoice );
		...
		public async Task< ContactAndAddressInfo > GetContactAndAddressInfo( s2cor__Sage_INV_Trade_Document__c saleInvoice )
		{
			var company = ( await this._asyncQueryManager.QueryOneAsync< s2cor__Sage_COR_Company__c >( SoqlQuery.Builder().Select( "Name" ).From( "s2cor__Sage_COR_Company__c" ).Where( "Id" ).IsEqualTo( saleInvoice.s2cor__Company__c ) ) ).GetValue( () => new s2cor__Sage_COR_Company__c() );
			var contact = ( await this._asyncQueryManager.QueryOneAsync< Contact >( SoqlQuery.Builder().Select( "Name", "MailingAddress" ).From( "Contact" ).Where( "Id").IsEqualTo( saleInvoice.s2cor__Contact__c ) ) ).GetValue( () => new Contact() );

			var contactInfo = new ContactInfo
			{
				FirstName = contact.FirstName ?? "N/A",
				LastName = contact.FirstName ?? "N/A",
